...
from src.config import FIGURES_DIR, SEASON_BY_MONTH, SEASON_ORDER

NAVY = "#17324D"
TEAL = "#167D8D"
GOLD = "#E6A54A"
GRAY = "#64748B"
COLORS = [TEAL, NAVY, GOLD, GRAY]
...
def _canvas(title: str, subtitle: str, ylabel: str) -> tuple:
    ...
    fig.text(.10, .035, "SYNTHETIC DATA  •  4-room independent guesthouse  •  2023–2025  •  Room revenue in EUR", color=GRAY, fontsize=8)
...
def create_visualizations(bookings: pd.DataFrame, tables: dict[str, pd.DataFrame]) -> None:
    ...
    specs = [
        ("revenue", "Monthly room revenue", "Revenue earned on each stay night; cancelled bookings excluded.", "Room revenue (EUR)", "monthly_revenue"),
        ("adr", "Average daily rate by month", "Room revenue divided by completed room nights; weighted across room types.", "ADR (EUR per sold room night)", "monthly_adr"),
        ("occupancy_rate", "Monthly room occupancy", "Completed room nights divided by 4 rooms × calendar days in each month.", "Occupancy rate", "monthly_occupancy"),
    ]
    ...
    for column, title, subtitle, ylabel, name in [
        ("revenue", "Room revenue by room type", "Four physical rooms: one of each type, with equal available room nights.", "Room revenue (EUR)", "room_type_revenue"),
        ("adr", "Achieved ADR by room type", "Completed room revenue divided by sold room nights within each room type.", "ADR (EUR per sold room night)", "room_type_adr"),
    ]:
        fig, ax = _canvas(title, subtitle, ylabel)
    ...
    fig, ax = _canvas(f"{seasonal['occupancy_rate'].idxmax()} season leads room occupancy", "Occupancy compares sold room nights with available inventory across all three years.", "Occupancy rate")
    bars = ax.bar(seasonal.index, seasonal["occupancy_rate"], color=[GRAY, TEAL, GOLD], width=.55)
    ...
    fig, ax = _canvas("Monthly occupancy and achieved ADR", "One point per month; shared synthetic seasonality does not establish a causal pricing effect.", "ADR (EUR per sold room night)")
    for season, color in zip(SEASON_ORDER, [GRAY, TEAL, GOLD]):
        months = [month for month, label in SEASON_BY_MONTH.items() if label == season]
    ...
    fig, ax = _canvas("Lead time and achieved room rates", "Weighted ADR by lead-time band and room type; season remains a confounder.", "ADR (EUR per sold room night)")
